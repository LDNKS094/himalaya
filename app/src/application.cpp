...
namespace himalaya::app {
    ...
    void Application::FrameStats::push(const float delta_time) {
    ...
    void Application::FrameStats::compute() {
    ...
    void Application::update() {
        // Debug panel (will be delegated to DebugUI in the next sub-task)
        frame_stats_.push(ImGui::GetIO().DeltaTime);
        ...
        ImGui::Separator();
        ImGui::Text("GPU: %s", context_.gpu_name.c_str());
        ImGui::Text("Resolution: %u x %u", swapchain_.extent.width, swapchain_.extent.height);

        const auto vram = context_.query_vram_usage();
        ImGui::Text("VRAM: %.1f / %.1f MB",
                    static_cast<double>(vram.used) / (1024.0 * 1024.0),
                    static_cast<double>(vram.budget) / (1024.0 * 1024.0));

        ImGui::Separator();
        if (ImGui::Checkbox("VSync", &swapchain_.vsync)) {
            vsync_changed_ = true;
        }
